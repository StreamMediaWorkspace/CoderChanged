...
class MainWindow(QMainWindow, updates.UpdateWatcher):
    ...
    def actionImportFiles_trigger(self, event):
        ...
        for file_path in files:
            ...
            self.filesTreeView.refresh_view()
            for file in File.filter(path=file_path):
                # Insert clip for this file at this position
                self.timeline.appendClip(file.id)
    ...
    def keyPressEvent(self, event):
        ...
        log.info("actionCut %s, %s", key.toString(), color)
        if color:
            self.timeline.cut(key.toString(), position, color)
        
        # Basic shortcuts i.e just a letter
        elif key.matches(self.getShortcutByName("seekPreviousFrame")) == QKeySequence.ExactMatch:
            # Pause video
