...
from classes.query import File, Clip, Transition, Track, Cut
...
class TimelineWebView(QWebView, updates.UpdateInterface):
    ...
    def addClip(self, data, position):
        ...
        new_clip["image"] = None#thumb_path
    ...
    def cut(self, key, position, color):
        log.info("add cut %s,%s, %s", key, position, color)

        #fps = get_app().project.get(["fps"])
        #print("======00000000======", json.dumps(get_app().project._data))
        #fps_num = self.timeline.getFpsNum()
        #fps_den = self.timeline.getFpsDen
    ...
    def update_cut_data(self, cut_json):
        ...
        # Search for matching clip in project data (if any)
        existing_cut = Cut.get(id=cut_data["id"])
        if existing_cut:
    ...
    def PlayCuts_Triggered(self, action, position, cuts):
        ...
        win = Cutting(cuts, preview=True)
        win.show()

    # Append Clip
    def appendClip(self, file_id):
        ...
        #yanght
        new_clip["layer"] = 0
        new_clip["position"] = self.lastPosition
        self.lastPosition = self.lastPosition + float(new_clip["reader"]["duration"])
