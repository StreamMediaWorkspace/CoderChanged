...
from windows.cutting import CutsToClips
    ...
    def accept(self):
        ...
        try:
            ...
            for cut in self.cuts:
                start_frame_export = round(float(cut["start"]) * fps_num / fps_den) - 1 
                end_frame_export = round(float(cut["end"]) * fps_num / fps_den) + 1
                progressstep = max(1 , round(end_frame_export- start_frame_export ) / 1000)
                start_time_export = time.time()
                # Write each frame in the selected range
                    ...
                    if (frame % progressstep) == 0:
                        ...
                        if ((( frame - start_frame_export ) != 0) & (( end_time_export - start_time_export ) != 0)):
                            ...
                            export_file_path =  _("%(hours)d:%(minutes)02d:%(seconds)02d Remaining (%(fps)5.2f FPS)") % { 'hours' : seconds_left / 3600,
                                                                                                                        ...
                                                                                                                        'seconds': seconds_left % 60,
                                                                                                                        'fps': fps_encode }
                        get_app().window.ExportFrame.emit(export_file_path, start_frame_export, end_frame_export, frame)
